...
def render(conn, usuario):
    ...
    with st.container(border=True):
        st.markdown(f"##### 👨‍👩‍👧‍👦 Grupo: {theme.esc(grupo['nome'])}")
        st.caption(f"ID do grupo: {grupo_id} · criado em {grupo['criado_em'][:10]}")

        with st.form("renomear_grupo"):
            novo_nome = st.text_input("Renomear grupo", value=grupo["nome"])
    ...
    with st.container(border=True):
        with st.form("add_membro", clear_on_submit=True):
            novo_email = st.text_input("E-mail", placeholder="pessoa@gmail.com")
    ...
    with st.container(border=True):
        with st.form("criar_grupo", clear_on_submit=True):
            nome_grupo = st.text_input("Nome do grupo", placeholder="Ex: Família João e Maria")
            email_admin = st.text_input(
                "E-mail do administrador do novo grupo",
            ...
            if st.form_submit_button("Criar grupo", use_container_width=True):
                if not nome_grupo.strip():
                    ...
                    st.success(
                        ...
                        f"Admin: {email_admin.strip().lower()}"
                    )
